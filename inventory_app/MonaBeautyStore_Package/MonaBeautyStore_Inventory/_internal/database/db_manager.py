...
class DatabaseManager:
    ...
    def get_product_by_id(self, product_id):
        """Get full product record by ID, including initial and current stock"""
        if not self.connect():
            return None

        try:
            self.cursor.execute('''
                SELECT p.id, p.name, p.sku, p.barcode, p.category_id, p.cogs,
    ...
    def update_product_initial_stock(self, product_id, new_initial_stock):
        """Update product initial_stock and align current_stock if current < 1 and there is no sales record.

        Note: We do not automatically change current_stock if there is existing stock movement history. This keeps
        accounting consistent. Caller may choose to also adjust current_stock separately when needed.
        """
        if not self.connect():
            return False
        ...
        try:
            ...
            self.cursor.execute('''
                UPDATE products
                SET initial_stock = ?, updated_at = ?
                WHERE id = ?
            ''', (new_initial_stock, current_time, product_id))

            self.connection.commit()
            return self.cursor.rowcount > 0
        except Exception as e:
            print(f"Error updating initial stock: {e}")
            return False
        finally:
            self.disconnect()
