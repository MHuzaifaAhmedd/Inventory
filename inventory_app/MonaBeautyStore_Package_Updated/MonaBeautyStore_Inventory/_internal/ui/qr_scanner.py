...
class QRScannerFrame(ttk.Frame):
    def __init__(self, parent, db_manager):
        ...
        self.action_frame = None  # Track the action buttons frame to prevent duplication
    ...
    def show_quick_actions(self):
        ...
        # Clear existing action frame if it exists to prevent duplication
        ...
        # Quick sale button
        sale_btn = ttk.Button(self.action_frame, text="💰 Quick Sale",
                             command=self.quick_sale)
        sale_btn.pack(side="left", padx=5)
        
        # Generate QR Code button
        qr_btn = ttk.Button(self.action_frame, text="🔲 Generate QR Code",
                            command=self.generate_product_qr_code)
        qr_btn.pack(side="left", padx=5)
        
        # Manual operation button
        manual_btn = ttk.Button(self.action_frame, text="⚙️ Manual Operations",
    ...
    def clear_results(self):
        ...
        # Also clear action buttons when clearing results
        if self.action_frame is not None:
            self.action_frame.destroy()
